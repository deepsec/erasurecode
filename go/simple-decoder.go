...
var dataShards = flag.Int("data", 6, "Number of shards to split the data into")
...
func main() {
	...
	if ok {
		...
		fmt.Println("**** ReconStruct Time: ", time.Since(reconstructStart).Seconds(), "(s) ****")
		...
		fmt.Println("**** Verity Time: ", time.Since(verifyStart).Seconds(), "(s) ****")
		checkErr(err)
	}
